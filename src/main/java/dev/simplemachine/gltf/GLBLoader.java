...
public final class GLBLoader {
    ...
    public Object load(byte[] array) {
        var byteBuffer = ByteBuffer.wrap(array).order(ByteOrder.LITTLE_ENDIAN);
        ...
        if (array.length != length) {
            Logger.getAnonymousLogger().info(String.format("File length is %d but header claims %d", array.length, length));
            return null;
        }
    ...
    public byte[] getData(GltfAccessor accessor) {
